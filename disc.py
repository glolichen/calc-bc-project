...
class Disc(ThreeDScene):
    ...
    def gen_riemann(self, function, a, b, subintervals, axes):
        ...
        for i in range(subintervals):
            xi = a + i * dx
            rect = self.gen_rectangle(axes, xi, 0, xi + dx, function(xi + dx), YELLOW_B)
            rects.append(rect)
        return Group(*rects)
    ...
    def construct(self):
        ...
        riemann = self.gen_riemann(weirdfunc, 0, 5, 5, axes)
        self.play(FadeIn(riemann), Write(riemann_text))

        # comment to remove riemann animation
        self.wait(1)
        self.remove(riemann)
        for i in range(6, 62, 5):
            riemann_new = self.gen_riemann(weirdfunc, 0, 5, i, axes)
        ...
        riemann_text_new.next_to(integral_text, DOWN, buff=0.2)
        self.play(Transform(riemann_text, riemann_text_new))
        # end comment to remove riemann animation

        self.wait(2)

        riemann_new = self.gen_riemann(weirdfunc, 0, 5, 10, axes)
